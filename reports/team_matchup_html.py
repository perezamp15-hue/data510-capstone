...
def _context_tables(report: dict[str, Any], pitcher_name: str) -> str:
    ctx = report.get("contextual_analysis") or {}
    def split_table(rows: list[dict[str, Any]], title: str) -> str:
        body = "".join(
            f"<tr><td>{escape(str(r.get('label','')))}</td><td>{r.get('pitches',0)}</td>"
            f"<td>{_value(r.get('velocity'))}</td><td>{_value(r.get('strike_rate'))}%</td>"
            f"<td>{_value(r.get('whiff_rate'))}%</td><td>{_value(r.get('hard_hit_rate'))}%</td>"
            f"<td>{_value(r.get('xwoba'),3)}</td></tr>" for r in rows
        )
        return f"<article><h4>{escape(title)}</h4><table><thead><tr><th>Split</th><th>Pitches</th><th>Velo</th><th>Strike</th><th>Whiff</th><th>Hard hit</th><th>xwOBA</th></tr></thead><tbody>{body or '<tr><td colspan=7>No qualified sample.</td></tr>'}</tbody></table></article>"
    compare = "".join(
        f"<tr><td>{escape(str(r.get('metric','')))}</td><td>{_value(r.get('season'),3 if r.get('metric')=='xwOBA' else 1)}{escape(str(r.get('suffix','')))}</td>"
        f"<td>{_value(r.get('recent'),3 if r.get('metric')=='xwOBA' else 1)}{escape(str(r.get('suffix','')))}</td>"
        f"<td>{_value(r.get('delta'),3 if r.get('metric')=='xwOBA' else 1)}</td></tr>"
        for r in ctx.get("self_comparison", [])
    )
    state = ctx.get("game_state") or {}
    return f"""
    <section class='context-section'><h3>{escape(pitcher_name)}: Current Form and Game Context</h3>
    <p><b>Scheduled location:</b> {escape(str(ctx.get('location_label','Not specified')))}. Recent form uses the last five games in the selected history window.</p>
    <div class='chart-grid'><article><h4>Season vs. last five games</h4><table><thead><tr><th>Metric</th><th>Season</th><th>Last 5</th><th>Change</th></tr></thead><tbody>{compare or '<tr><td colspan=4>No comparison sample.</td></tr>'}</tbody></table></article>
    {split_table(ctx.get('home_away', []), 'Home / away')}</div>
    <div class='chart-grid'>{split_table(state.get('runners', []), 'Runner state')}{split_table(state.get('innings', []), 'Early / middle / late')}</div>
    {split_table(state.get('times_through_order', []), 'Times through the order')}
    </section>"""


def _peer_section(report: dict[str, Any], pitcher_name: str) -> str:
    peer=report.get("peer_comparison") or {}
    similar="".join(f"<tr><td>{escape(str(r.get('pitcher_name','')))}</td><td>{_value(r.get('velocity'))}</td><td>{_value(r.get('whiff_rate'))}%</td><td>{_value(r.get('strike_rate'))}%</td><td>{_value(r.get('xwoba'),3)}</td><td>{_value(r.get('distance'),3)}</td></tr>" for r in peer.get("similar",[]))
    ranks="".join(f"<div class='metric'><span>{escape(str(r.get('metric','')))}</span><strong>{_value(r.get('percentile'),0)}th pct</strong><small>{_value(r.get('value'),3)}</small></div>" for r in peer.get("rankings",[]))
    return f"""<section><h3>{escape(pitcher_name)} Peer Comparison and Rankings</h3><div class='dashboard'>{ranks or '<p>No qualified league comparison.</p>'}</div><h4>Most similar pitcher profiles</h4><table><thead><tr><th>Pitcher</th><th>Velo</th><th>Whiff</th><th>Strike</th><th>xwOBA</th><th>Distance</th></tr></thead><tbody>{similar or '<tr><td colspan=6>No similar-pitcher sample.</td></tr>'}</tbody></table></section>"""


def _bullpen_section(report: dict[str, Any], team_name: str) -> str:
    rows="".join(f"<tr><td>{escape(str(r.get('pitcher_name','')))}</td><td>{escape(str(r.get('throws','')))}</td><td>{r.get('appearances',0)}</td><td>{r.get('pitches',0)}</td><td>{_value(r.get('velocity'))}</td><td>{_value(r.get('whiff_rate'))}%</td><td>{_value(r.get('xwoba'),3)}</td></tr>" for r in report.get("bullpen",[]))
    return f"""<section><h3>{escape(team_name)} Bullpen Snapshot</h3><p class='note'>Recent workload and pitch quality from the latest 21-day window in the selected data. Confirm actual availability before the game.</p><table><thead><tr><th>Reliever</th><th>Throws</th><th>Appearances</th><th>Pitches</th><th>Velo</th><th>Whiff</th><th>xwOBA</th></tr></thead><tbody>{rows or '<tr><td colspan=7>No bullpen sample available.</td></tr>'}</tbody></table></section>"""


def _outcome_model_section(report: dict[str, Any], pitcher_name: str) -> str:
    model=report.get("pitch_outcome_model") or {}
    rows="".join(f"<tr><td>{escape(str(r.get('pitch_type','')))}</td><td>{escape(str(r.get('zone','')))}</td><td>{_value(r.get('expected_run_value'),3)}</td><td>{_value(r.get('whiff'))}%</td><td>{_value(r.get('called_strike'))}%</td><td>{_value(r.get('hard_contact'))}%</td></tr>" for r in model.get("candidate_scores",[]))
    return f"""<section><h3>{escape(pitcher_name)} Pitch-Outcome Recommender</h3><p><b>Status:</b> {'Active' if model.get('available') else 'Not active'} · <b>Classified pitches:</b> {model.get('sample_size',0)} · <b>Accuracy:</b> {_value(model.get('accuracy'),3)} · <b>Log loss:</b> {_value(model.get('log_loss'),3)}</p><p class='note'>{escape(str(model.get('note','')))} Lower expected run value is preferred.</p><table><thead><tr><th>Pitch</th><th>Target</th><th>Expected run value</th><th>Whiff</th><th>Called strike</th><th>Hard contact</th></tr></thead><tbody>{rows or '<tr><td colspan=6>Outcome recommendations need a larger classified sample.</td></tr>'}</tbody></table></section>"""
...
<nav><a href="#summary">Summary</a><a href="#offense">Offense vs. {escape(opposing_pitcher)}</a><a href="#defense">{escape(our_pitcher)} plan</a><a href="#context">Context</a><a href="#bullpen">Bullpen</a><a href="#method">Method</a></nav>
...
{offense_cards}
<section id="context">{_context_tables(offense, opposing_pitcher)}{_peer_section(offense, opposing_pitcher)}{_outcome_model_section(offense, opposing_pitcher)}</section>
...
{defense_cards}
<section id="context-defense">{_context_tables(defense, our_pitcher)}{_peer_section(defense, our_pitcher)}{_outcome_model_section(defense, our_pitcher)}</section>
<section id="bullpen">{_bullpen_section(offense, opponent_team)}{_bullpen_section(defense, our_team)}</section>
<section id="method"><h2>Method and limitations</h2><p>The same deterministic matchup engine is run twice. First, it models the opposing pitcher against our batting order so hitters can anticipate his strongest matchup choices. Second, it models our pitcher against the opposing batting order to create the defensive pitch plan. Recommendations combine arsenal performance, hitter history by pitch type, count context, target-location rules, sample-size regression, hot/cold zones, movement, release consistency, tunneling compatibility, recent five-game form, home/away splits, runner state, times through the order, comparable-pitcher distance, bullpen workload and a separate pitch-outcome model.</p><p class="note">Historical decision support only; recommendations are not guaranteed outcomes. Preset lineups are editable examples and should be replaced with the confirmed batting orders for a specific game.</p></section></main>
