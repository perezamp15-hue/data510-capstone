"""Build pitch-sequence training features."""
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Build pitch-sequence model features."
    )
    ...
    parser.add_argument(
        ...
        default=7,
        help=(
            "When dates are omitted, rebuild this many days "
            "before today through today. Default: 7."
        ),
    )
...
def resolve_dates(
    ...
    """Resolve the requested feature-build range."""
...
def main() -> int:
    ...
    print(f"Source pitches:  {result.source_pitches}")
    print(f"Rows deleted:    {result.rows_deleted}")
    print(f"Rows available:  {result.rows_inserted}")

    if result.source_pitches == 0:
        print(
            "\nWarning: no source pitches existed in this date range."
        )
