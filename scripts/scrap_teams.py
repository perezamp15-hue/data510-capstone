...
def run():
    ...
    try:
        ...
        parsed = []
        for t in raw_teams:
            if t.get('sport', {}).get('id') != 1: continue
            parsed.append({
        ...
        with engine.begin() as conn:
            for _, row in df.iterrows():
                conn.execute(text("""
                    ...
                    ON CONFLICT (team_id) DO UPDATE SET abbreviation = EXCLUDED.abbreviation, team_name = EXCLUDED.team_name, city = EXCLUDED.city, nickname = EXCLUDED.nickname, league = EXCLUDED.league, division = EXCLUDED.division;
                """), row.to_dict())
