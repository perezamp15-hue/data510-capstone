...
def run(target_date=None):
    ...
    try:
        ...
        for tx in raw_txs:
            ...
            from_t, to_t = tx.get('fromTeam', {}).get('id'), tx.get('toTeam', {}).get('id')
            parsed.append({
                "player_id": int(p_id), "transaction_date": tx.get('date'), "transaction_type": tx.get('typeDesc', 'Roster Move'),
                "from_team_id": int(from_t) if from_t and int(from_t) in valid_teams else None,
                "to_team_id": int(to_t) if to_t and int(to_t) in valid_teams else None, "injury_status": tx.get('description', '')[:254]
            })
