            ...
            try:
                conn.execute(text("""
                    INSERT INTO statcast_pitches (
                        ...
                        ball_count, strike_count, batter_id, pitcher_id, pitch_type, 
                        pitch_code, pitch_description, -- Explicit Outcome Fields Added Here
                        release_velocity, release_spin_rate,
                        ...
                        :game_pk, :game_date, :pa_num, :ab_num, :pitch_num, :inn, :half, :outs, :balls, :strikes, :bat_id, :pit_id, :p_type, 
                        :pitch_code, :pitch_description, -- Bound Variable Remapping Hooks
                        :vel, :spin,
                ...
                """), {
                    ...
                    # Maps 'type' (B, S, X) and 'description' (Ball, Called Strike, Swinging Strike...) safely
                    "pitch_code": str(row['type']) if pd.notna(row['type']) else None,
                    "pitch_description": str(row['description']) if pd.notna(row['description']) else None,
                    ...
                    "on_1b": pd.notna(row['on_1b']) and row['on_1b'] != 0 and pd.notna(row['on_1b']), 
                    "on_2b": pd.notna(row['on_2b']) and row['on_2b'] != 0 and pd.notna(row['on_2b']), 
                    "on_3b": pd.notna(row['on_3b']) and row['on_3b'] != 0 and pd.notna(row['on_3b']),
                ...
                # Shield individual loop iteration errors to keep batch pipeline from halting
                continue
