...
# Explicitly ensure your data directory exists before running
...
def collect_stadium_registry():
    ...
    df.to_csv("data/raw/stadium_registry.csv", index=False)
    return stadium_data
...
def fetch_schedules_with_lineups(years=[2024]):
                ...
                for date_node in data.get("dates", []):
                    for game in date_node.get("games", []):
                        ...
                        game_item = {
                            ...
                            "starting_lineups": {"home_batters": [], "away_batters": [], "home_starting_pitcher": None, "away_starting_pitcher": None}
                        }
    ...
    with open("data/raw/mlb_games_with_players_raw.json", "w") as f:
...
def fetch_hourly_weather(games_list):
    ...
    for game in games_list[:15]:
        ...
        if home_team in team_coords:
            ...
            weather_url = f"https://archive-api.open-meteo.com/v1/archive?latitude={lat}&longitude={lon}&start_date={date_str}&end_date={date_str}&hourly=temperature_2m,relative_humidity_2m,wind_speed_10m,wind_direction_10m&temperature_unit=fahrenheit&wind_speed_unit=mph"
            try:
                response = requests.get(weather_url, timeout=10)
                if response.status_code == 200:
                    weather_records.append({"game_pk": game["game_pk"], "game_date": date_str, "home_team": home_team, "hourly_weather": response.json().get("hourly", {})})
                time.sleep(0.2)
            except Exception:
                pass
    with open("data/raw/weather_hourly_raw.json", "w") as f:
        json.dump(weather_records, f, indent=4)

if __name__ == "__main__":
    collect_stadium_registry()
    sched = fetch_schedules_with_lineups()
    fetch_statcast_events()
    if sched:
        fetch_hourly_weather(sched)
