                    ...
                    if "batting" in stats and stats["batting"] != {}:
                        ...
                        if bat_stats.get("atBats", 0) > 0 or bat_stats.get("plateAppearances", 0) > 0:
                            game_log_entry[role]["batters"].append({
                                ...
                                "team_name": current_team_name, "position": player_info.get("position", {}).get("abbreviation"),
                                "at_bats": bat_stats.get("atBats"), "runs": bat_stats.get("runs"), "hits": bat_stats.get("hits"),
                                "rbi": bat_stats.get("rbi"), "walks": bat_stats.get("baseOnBalls"), "strikeouts": bat_stats.get("strikeOuts")
                            })
                    ...
                    if "pitching" in stats and stats["pitching"] != {}:
                        ...
                        if pitch_stats.get("pitchesThrown", 0) > 0:
                            game_log_entry[role]["pitchers"].append({
                                "player_id": person.get("id"), "name": person.get("fullName"),
                                "team_name": current_team_name, "innings_pitched": pitch_stats.get("inningsPitched"),
                                "hits_allowed": pitch_stats.get("hits"), "runs_allowed": pitch_stats.get("runs"),
                                "earned_runs": pitch_stats.get("earnedRuns"), "strikeouts": pitch_stats.get("strikeOuts"),
                                "pitches_thrown": pitch_stats.get("pitchesThrown")
                            })
                            
            all_game_logs[str(game_pk)] = game_log_entry
        except Exception as e:
            print(f"Error parsing log mapping for Game {game_pk}: {e}")

    output_path = "data/raw/detailed_game_logs.json"
    with open(output_path, "w") as f:
        json.dump(all_game_logs, f, indent=4)
    print(f"-> Success: Role game logs saved to {output_path}.\n")

if __name__ == "__main__":
    extract_detailed_game_logs(game_pk_list=[747031])
